...
from C6.demo_mongo.database import get_database
from C6.demo_mongo.models import Post, PostCreate, PostPartialUpdate, CommentCreate
...
async def create_post(
    ...
    post = await get_post_or_404(id=post.id, database=database)
    return post
...
async def list_posts(
    ...
    results = [Post(**raw_posts) async for raw_posts in query]
    return results
...
async def update_post(
    ...
    await database['posts'].update_one({"_id": post.id}, {"$set": post_update_dict})
    post = await get_post_or_404(id=post.id, database=database)
    return post
